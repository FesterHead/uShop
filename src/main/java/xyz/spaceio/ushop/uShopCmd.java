...
public class uShopCmd implements CommandExecutor {
          ...
          if (p.getInventory().getItemInMainHand() != null) {
            if (p.getInventory().getItemInMainHand().getType() != null) {
              if (p.getInventory().getItemInMainHand().getType() != Material.AIR) {
                ...
                if (args.length > 2) {
                  ...
                  for (int i = 2; i < args.length; i++) {
                    ...
                    try {
                      ...
                      List<String> flags = Arrays.stream(Flags.values())
                          .map(flag -> flag.name().toLowerCase()).collect(Collectors.toList());
                ...
                p.sendMessage(
                    plugin.getCustomItemDescription(customItem, 1).stream().toArray(String[]::new));
                return true;
              }
            }
          }
  ...
  private void showHelp(CommandSender cs) {
    ...
    cs.sendMessage(
        "§e/ushop §aadd <price> [flags ...] §r- sets a custom price for an item with custom lore, displayname, durability and enchants");
    cs.sendMessage("§e/ushop §aopen <player> §r- opens the shop for other players");
    cs.sendMessage(
        "§e/ushop §aconvert §r- will convert your essentials worth list to the ushop one");
    cs.sendMessage(
        "§cCurrently configured custom items (with NBT Data): §a" + plugin.getCustomItemCount());
  }

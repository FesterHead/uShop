  ...
  public CustomItem(ItemStack is, double price) {
    ...
    if (is.getEnchantments() != null) {
      ...
      this.enchantements = is.getEnchantments().entrySet().stream()
          .collect(Collectors.toMap(x -> x.getKey().getKey().getKey(), x -> x.getValue()));
    }
  }
  ...
  public boolean matches(ItemStack is) {
    ...
    if (hasMeta && !hasFlag(Flags.IGNORE_META)) {
      ...
      if (!hasFlag(Flags.IGNORE_DISPLAYNAME)) {
        if (displayname == null && is.getItemMeta().hasDisplayName()
            || displayname != null && !is.getItemMeta().hasDisplayName()) {
          return false;
        }

        if (displayname != null && is.getItemMeta().hasDisplayName()
            && !displayname.equals(is.getItemMeta().getDisplayName())) {
          return false;
        }
      }
